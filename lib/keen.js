...
module.exports = {
  ...
  recordKeenEvent: (eventCollection, eventBody, context) => {
    ...
    const keenWriteKey = context.secrets.KEEN_WRITE_API_KEY;
    ...
    return new Promise((resolve, reject) => {
      ...
      keenClient.recordEvent(eventCollection, eventBody, function(error, res) {
        if (error) {
          console.error('KeenAPI Error', error);
          reject(error);
        } else {
          console.error('KeenAPI Success');
          resolve(res);
        }
      });
    });
  ...
  runKeenQuery: (operation, properties, context) => {
    const keenProjectId = context.secrets.KEEN_PROJECT_ID;
    const keenReadKey = context.secrets.KEEN_READ_API_KEY;
    var keenClient = new KeenAnalysis({
      projectId: keenProjectId, readKey: keenReadKey
    });
    return keenClient.query(operation, properties);
  },

  userForEvent: (apiResponse, context) => {
    let user = apiResponse.user;
    let baseUser = Object.assign(
      pick(user, [
    ...
    // put these values in your secrets file if you'd like to
    // have specific flags for activity by certain groups
    if (context.secrets.EMPLOYEES_DISCOURSE_GROUP_ID) {
      baseUser.is_employee = typeof find(user.groups, { id: parseInt(context.secrets.EMPLOYEES_DISCOURSE_GROUP_ID) }) === 'object';
    }
    if (context.secrets.AMBASSADORS_DISCOURSE_GROUP_ID) {
      baseUser.is_ambassador = typeof find(user.groups, { id: parseInt(context.secrets.AMBASSADORS_DISCOURSE_GROUP_ID) }) === 'object';
    }
    return baseUser;
  },
